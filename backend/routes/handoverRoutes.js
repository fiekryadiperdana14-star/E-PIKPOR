...
router.post('/', async (req, res) => {
    ...
    try {
        ...
        await conn.query(
            'INSERT INTO handovers (report_id, regu_pengirim_id, regu_penerima_id, catatan, status_terima) VALUES (?, ?, ?, ?, ?)',
...
router.put('/:id', async (req, res) => {
    ...
    try {
        ...
        await conn.query(
            'UPDATE handovers SET status_terima = ? WHERE id = ?',
        ...
        if (status_terima === 'diterima') {
            await conn.query('UPDATE reports SET status = ? WHERE id = ?', ['selesai', report_id]);
        } else if (status_terima === 'ditolak') {
            await conn.query('UPDATE reports SET status = ? WHERE id = ?', ['pending', report_id]);
        }
